...
WAV=/usr/local/share/media/wetsquish.wav
...
RES=`psql -d alarms_mqtt -c "select sound, notify from mode where description = 'window close';" -t -q`
...
if [ $NOTIFY == 't' ]; then
        echo $1 | mailx -s "window close" $DIST
fi
