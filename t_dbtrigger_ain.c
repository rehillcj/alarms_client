...
void t_dbtrigger_ain(s_data_t *data) {
	...
	if (PQstatus(conn_app) != CONNECTION_OK) {
		memset(printbuff, 0, SIZE_PRINTBUFF);
		sprintf(printbuff, "exiting on error, connecting to hvac database in 't_dbtrigger_ain' failed: %s", PQerrorMessage(conn_app));
		do_print(data, printbuff, RED);
		PQfinish(conn_app);
	...
	while(!data->error){
		...
		while ((notify = PQnotifies(conn_arduino)) != NULL) {
			...
			do_print(data, printbuff, YELLOW);
	        			...
	        			//pthread_mutex_lock(pmut_sql_app);
        				memset(sqlbuff, 0, sizeof(sqlbuff));
	        			sprintf(sqlbuff, "update sensor set state = '%.2f', date_time = now() where unit = '%s' and port = 'a%02d'", atof(tbuff), PQgetvalue(res1, 0, 1), k);
