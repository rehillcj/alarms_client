...
void t_processloop(s_data_t *data)
{
	...
	if (PQstatus(conn_app) != CONNECTION_OK) {
		memset(printbuff, 0, SIZE_PRINTBUFF);
		sprintf(printbuff, "exiting on error code 4, connection to alarms database in 't_processloop' failed: %s", PQerrorMessage(conn_app));
		do_print(data, printbuff, RED);
		PQfinish(conn_app);
	...
	while(data->quit == FALSE){
		...
		res1 = PQexec(conn_app, "select * from actuator order by db, id");
		...
		pthread_mutex_lock(pmut_sql_app);
		res1 = PQexec(conn_app, "select * from contact order by db, id");
		pthread_mutex_unlock(pmut_sql_app);
		for (i = 0; i < PQntuples(res1); i++) {
			memset(tbuff, 0, sizeof(tbuff));
			sprintf(tbuff, "select %s from status_dio where unit = (select id from unit where name = '%s' and online = 't');", PQgetvalue(res1, i, 4), PQgetvalue(res1, i, 3));
