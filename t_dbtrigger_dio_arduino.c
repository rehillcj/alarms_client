...
void t_dbtrigger_dio_arduino(s_data_t *data) {
	...
	if (PQstatus(conn_app) != CONNECTION_OK) {
		memset(printbuff, 0, SIZE_PRINTBUFF);
		sprintf(printbuff, "exiting on error, connecting to hvac database in 't_dbtrigger_dio_arduino' failed: %s", PQerrorMessage(conn_app));
		do_print(data, printbuff, RED);
		PQfinish(conn_app);
			...
			if (strlen(notifybuff) > 0) {
				...
				i++;
        	                ...
        	                while (notifybuff[i] != '|') {
                        	        ...
                        	        //memset(sqlbuff, 0, sizeof(sqlbuff));
                        	        //sprintf(sqlbuff, "update actuator set state = '%c', date_time = now() where unit = '%s' and port = 'd%02d' and db='arduino_mqtt';", notifybuff[i], PQgetvalue(res1, 0, 1), k);
	                                //res2 = PQexec(conn_app, sqlbuff);
        	                        //PQclear(res2);

                	                memset(sqlbuff, 0, sizeof(sqlbuff));
                        	        sprintf(sqlbuff, "update contact set state = '%c', date_time = now() where unit = '%s' and port = 'd%02d' and db = 'arduino_mqtt';", notifybuff[i], PQgetvalue(res1, 0, 1), k);
                        	        res2 = PQexec(conn_app, sqlbuff);
	                                PQclear(res2);
