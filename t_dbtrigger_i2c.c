...
void t_dbtrigger_i2c(s_data_t *data) {
	...
	if (PQstatus(conn_arduino) != CONNECTION_OK) {
		...
		sprintf(printbuff, "exiting on error, connecting to arduino database in 't_dbtrigger_i2c' failed: %s", PQerrorMessage(conn_arduino));
	...
	if (PQstatus(conn_app) != CONNECTION_OK) {
		memset(printbuff, 0, SIZE_PRINTBUFF);
		sprintf(printbuff, "exiting on error, connecting to hvac database in 't_dbtrigger_i2c' failed: %s", PQerrorMessage(conn_app));
		do_print(data, printbuff, RED);
		PQfinish(conn_app);
        ...
        do_print(data, printbuff, RED);
	res1 = PQexec(conn_arduino, "LISTEN status_i2c_arduino_mqtt");
		...
		while ((notify = PQnotifies(conn_arduino)) != NULL) {
			...
			if (strlen(notifybuff) > 0) {
				...
				i++;
        				...
        				memset(sqlbuff, 0, sizeof(sqlbuff));
	        			sprintf(sqlbuff, "update sensor set state = '%.2f', date_time = now() where unit = '%s' and port = 'i%02d'", atof(tbuff), PQgetvalue(res1, 0, 1), k);
	        			res2 = PQexec(conn_app, sqlbuff);
	        			PQclear(res2);
        				memset(printbuff, 0, sizeof(printbuff));
	        			sprintf(printbuff, "port: i%02d\tstate: %.2f", k, atof(tbuff));
		        		do_print(data, printbuff, GREEN);
		        		i++;
	...
	memset(printbuff, 0, sizeof(printbuff));
	sprintf(printbuff, "exiting thread: dbtrigger_i2c");
