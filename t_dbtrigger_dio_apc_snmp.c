...
void t_dbtrigger_dio_apc_snmp(s_data_t *data) {
	...
	if (PQstatus(conn_app) != CONNECTION_OK) {
		memset(printbuff, 0, SIZE_PRINTBUFF);
		sprintf(printbuff, "exiting on error, connecting to application database in 't_dbtrigger_dio_apc_snmp' failed: %s", PQerrorMessage(conn_app));
		do_print(data, printbuff, RED);
		PQfinish(conn_app);
		exit(ERROR_DB_CONNECT_APP);
	}
        memset(printbuff, 0, SIZE_PRINTBUFF);
        sprintf(printbuff, "t_dbtrigger_dio_apc_snmp thread libpq connection to %s, fd: %i", tbuff, PQsocket(conn_app));
        do_print(data, printbuff, RED);
	...
	while(!data->error){
		...
		while ((notify = PQnotifies(conn_apc_snmp)) != NULL) {
			...
			res1 = PQexec(conn_apc_snmp, sqlbuff);
                        ...
                        while (notifybuff[i] != '|') {
                                memset(sqlbuff, 0, sizeof(sqlbuff));
                                sprintf(sqlbuff, "update actuator set state = '%c', date_time = now() where unit = '%s' and port = 'd%02d' and db='apc_snmp'", notifybuff[i], PQgetvalue(res1, 0, 1), k);
